...
**Last updated:** 2026-07-13 (Phase 16.2 built — scheduler, schedule API, default-on, bus events + failure emails; all automated checks pass. Next: 16.3 web UI + ops docs.)
...
**Phase 16.1 + 16.2 (Backup server) are built and pass all automated checks.** 16.1 (2026-07-08, reviewed + tested 07-09): `internal/backup` engine/manager (`VACUUM INTO` + integrity check, filename-is-the-record history, traversal-guarded delete), superadmin `/admin/backup*` endpoints — Docker-smoke-verified by `/test-phase 16.1`. 16.2 (2026-07-13): `Scheduler` (preset → `NextRun` in UTC, injected clock, `Reload` poke, skip-while-running, no catch-up), `GET/PUT /admin/backup/schedule` (`backup.schedule` `instance_settings` JSON key, echoes `nextRunAt`, status `schedule` stub now live), **default-on daily 02:00 UTC / keep-14**, `backup.completed`/`backup.failed` bus events (instance-scoped, empty TeamID — invisible to WS clients) + `Notifier` emailing superadmins on failure (silent no-op without SMTP), `SetKeepLast` restored as atomic, `BackupSchedule` OpenAPI schema + regenerated TS types. Not yet run for real against Docker — that's 16.3's live verification. Detail in [log.md](../log.md).

| Next phase | Scope | Plan |
|------------|-------|------|
| **16.3** | Settings › Backup UI (status card + health badge, Back up now, schedule form with next-run, history table), `useBackup*` hooks + component tests, `docs/OPERATIONS.md` restore runbook + volume contract, live Docker verification incl. walking the runbook once for real, `/test-phase 16`, TESTING.md Phase 16 assertions. | [docs/plans/phase-16-backup.md](../plans/phase-16-backup.md) §16.3 |
