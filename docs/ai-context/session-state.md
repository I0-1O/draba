...
**Last updated:** 2026-07-01 (Phase 14.4 — printable views + HTML save, both reusing the 14.3 `PresentationFrame` surface; Calendar gained a clean `interactive=false` renderer; the two remaining "force light" implementations collapsed into `lib/presentationTheme.ts`. Live-verified against the real Docker-backed API in dark mode — Phase 14 is now fully done.)
