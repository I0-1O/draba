...
**Last updated:** 2026-07-03 (Phase 15.1 built — server half of import: pure `internal/importer` package (parse/map/dates/resolve per the plan's tolerance contract), `POST /teams/:id/timelines/:timelineId/import` stateless two-pass endpoint, `GET /import/template.csv|.xlsx`, `ActivityRepo.CreateImportBatch` single-tx commit, OpenAPI schemas + regenerated TS types. All automated checks pass incl. the export→import round-trip test. Next: 15.2 wizard.)
...
**Phase 15.1 (Import — server) is built and passes all automated checks (2026-07-03).** Pure `internal/importer` package implementing the full tolerance contract (delimiter/BOM/cp1252 sniffing, header synonyms + explicit mapping override, column-wide date-order resolution + Excel serials, warn+skip name resolution, in-file-first parent matching with cycle breaking, duplicate disclosure), stateless two-pass endpoint (`dryRun` preview provably read-only; commit = one `CreateImportBatch` transaction, parents before children, `ActivityCreated` events after), template endpoints served from `export.Columns`. Table-driven tests cover every rule; the round-trip test (14.1 CSV/xlsx export → re-import → same activities modulo IDs) passes. Detail in [log.md](../log.md).

| Next phase | Scope | Plan |
|------------|-------|------|
| **15.2** | Import wizard (web): stepped dialog off the sidebar "Bulk import" stub — upload → conditional map-columns step → preview table (per-cell messages, All/Warnings/Errors chips, "Create N missing tags" checkbox) → commit + result. `useImportPreview`/`useCommitImport` hooks on the same endpoint. Then 15.3 hardening (messy-file corpus e2e, Docker verification, TESTING.md). | [docs/plans/phase-15-import.md](../plans/phase-15-import.md) §15.2 |
