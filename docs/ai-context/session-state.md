...
| **16** | Backup & Restore — **planned 2026-07-08**, scope settled, TASKS.md populated. SQLite-only (`VACUUM INTO` + integrity check; MySQL/Postgres cut — no drivers exist), history = directory scan (no DB table), `DRABA_BACKUP_DIR` default `/data/backups`, preset scheduler (first background scheduler in the codebase) default-on daily 02:00 / keep-14, no HTTP download / no in-app restore (runbook) / no encryption for v1. Three sub-phases: 16.1 engine + manual, 16.2 scheduler + notification, 16.3 web UI + OPERATIONS.md. | [docs/plans/phase-16-backup.md](../plans/phase-16-backup.md) |
