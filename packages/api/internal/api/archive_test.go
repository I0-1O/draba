...
func TestArchiveEvent_HiddenByDefaultRestorableWithFlag(t *testing.T) {
	srv, token, teamID := eventTestSetup(t)

	// Create an event.
	wc := httptest.NewRecorder()
	srv.ServeHTTP(wc, authReq(http.MethodPost, fmt.Sprintf("/teams/%s/events", teamID), map[string]any{
		"title":   "To archive",
	...
	require.NoError(t, json.NewDecoder(wc.Body).Decode(&evt))
	eventID := evt["id"].(string)

	// Archive.
	wa := httptest.NewRecorder()
	srv.ServeHTTP(wa, authReq(http.MethodPost, "/events/"+eventID+"/archive", nil, token))
	...
	// Default list excludes archived events.
	wl := httptest.NewRecorder()
	srv.ServeHTTP(wl, authReq(http.MethodGet, fmt.Sprintf("/teams/%s/events", teamID), nil, token))
	...
	assert.Empty(t, defaultList, "archived event hidden from default list")

	// ?archived=true restores visibility.
	wl2 := httptest.NewRecorder()
	srv.ServeHTTP(wl2, authReq(http.MethodGet, fmt.Sprintf("/teams/%s/events?archived=true", teamID), nil, token))
	...
	assert.Equal(t, eventID, withArchived[0]["id"])

	// Unarchive.
	wu := httptest.NewRecorder()
	srv.ServeHTTP(wu, authReq(http.MethodPost, "/events/"+eventID+"/unarchive", nil, token))
	...
	srv.ServeHTTP(wl3, authReq(http.MethodGet, fmt.Sprintf("/teams/%s/events", teamID), nil, token))
