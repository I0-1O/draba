...
func timelineTestSetup(t *testing.T) (srv http.Handler, aliceToken, teamID string) {
	...
	srv = api.NewServer(users, invites, teams, activitiesRepo, timelinesRepo, db.NewSavedFilterRepo(database), db.NewUserPreferenceRepo(database), db.NewAPITokenRepo(database), isrTl, db.NewPasswordResetTokenRepo(database), db.NewStatusRepo(database), db.NewTagRepo(database), mailer.New(isrTl, nil), tokens, tier.Unlimited, bus, hub).Routes()
...
func TestGetTimeline_MemberGrantedAccessAllowed(t *testing.T) {
	...
	srv := api.NewServer(users, invites, teams, activitiesRepo, timelinesRepo,
		db.NewSavedFilterRepo(database), db.NewUserPreferenceRepo(database), db.NewAPITokenRepo(database), isrAcc, db.NewPasswordResetTokenRepo(database), db.NewStatusRepo(database), db.NewTagRepo(database), mailer.New(isrAcc, nil), tokens, tier.Unlimited, bus, hub).Routes()
...
func TestCreateTimeline_RestrictedGrantAccessError(t *testing.T) {
	...
	srv := api.NewServer(users, invites, teams, activitiesRepo, fake, db.NewSavedFilterRepo(database), db.NewUserPreferenceRepo(database), db.NewAPITokenRepo(database), isrFake, db.NewPasswordResetTokenRepo(database), db.NewStatusRepo(database), db.NewTagRepo(database), mailer.New(isrFake, nil), tokens, tier.Unlimited, bus, hub).Routes()
...
func TestCreateTimeline_PublishesBusMessage(t *testing.T) {
	...
	srv := api.NewServer(users, invites, teams, activitiesRepo, timelinesRepo, db.NewSavedFilterRepo(database), db.NewUserPreferenceRepo(database), db.NewAPITokenRepo(database), isrBus, db.NewPasswordResetTokenRepo(database), db.NewStatusRepo(database), db.NewTagRepo(database), mailer.New(isrBus, nil), tokens, tier.Unlimited, bus, hub).Routes()
