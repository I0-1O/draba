...
func savedFilterTestSetup(t *testing.T) (srv http.Handler, aliceToken, teamID string) {
	...
	srv = api.NewServer(users, invites, teams, activitiesRepo, timelinesRepo, savedFiltersRepo, db.NewUserPreferenceRepo(database), db.NewAPITokenRepo(database), isrSF, db.NewPasswordResetTokenRepo(database), mailer.New(isrSF), tokens, tier.Unlimited, bus, hub).Routes()
